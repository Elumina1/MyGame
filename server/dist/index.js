...
const cards = (items) => items.map((v, i) => {
    const [visual, name] = v.split("|");
    return { id: String(i + 1), name, visual };
});
const dotaHeroes = createRequire(import.meta.url)(fileURLToPath(new URL("../src/dotaHeroes.json", import.meta.url)));
const dotaCards = dotaHeroes.map((hero) => ({ ...hero, visual: "⚔️" }));
export const themes = [
    {
        id: "animals",
        name: "Животные",
...
const validCode = (v) => typeof v === "string" && /^[A-Z0-9]{5}$/.test(v.trim().toUpperCase())
    ? v.trim().toUpperCase()
    : "";
const validName = (v) => typeof v === "string" && v.trim().length >= 2 && v.trim().length <= 20
    ? v.trim()
    : "";
const validText = (v) => typeof v === "string" && v.trim() && v.trim().length <= 300 ? v.trim() : "";
const getTheme = (id) => themes.find((t) => t.id === id);
const publicState = (r, token) => {
...
const fields = (value) => value !== null && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
const auth = (s, t) => {
    const r = rooms.get(s.data.room), p = r?.players.find((x) => x.token === t);
...
io.on("connection", (s) => {
    s.on("create", (raw) => {
        ...
        const safeToken = validToken(token), name = validName(nickname), theme = getTheme(themeId);
        if (!safeToken || !name)
            return fail(s, "Имя или идентификатор игрока некорректны.");
        if (!theme)
            return fail(s, "Неизвестная тема.");
        const cardIds = (theme.id === "dota2" ? selectRandomCards(theme.cards, 24) : theme.cards).map((card) => card.id);
        const r = {
            code: makeCode(),
            themeId: theme.id,
            cardIds,
    ...
    s.on("join", (raw) => {
        ...
        const roomCode = validCode(code), safeToken = validToken(token), r = rooms.get(roomCode), name = validName(nickname);
        if (!roomCode || !r)
            return fail(s, "Комната не найдена.");
        if (!safeToken || !name)
            return fail(s, "Имя или идентификатор игрока некорректны.");
        let p = r.players.find((x) => x.token === safeToken);
        if (!p && r.players.length >= 2)
            return fail(s, "Комната заполнена.");
        if (!p) {
    ...
    s.on("ready", (raw) => {
        ...
        const x = auth(s, token);
        if (!x || x.r.phase !== "selecting" || !x.p.secret)
            return;
        x.p.ready = true;
    ...
    s.on("askQuestion", (raw) => {
        const { token, question } = fields(raw);
        const x = auth(s, token), q = validText(question);
    ...
    s.on("answerQuestion", (raw) => {
        ...
        if (!round || round.answer)
            return fail(s, "На этот вопрос уже ответили.");
        round.answer = answer;
        x.r.turn = x.p.token;
        x.r.phase = "waiting_for_question";
        broadcast(x.r);
    ...
    s.on("disconnect", () => {
        const r = rooms.get(s.data.room), p = r?.players.find((x) => x.socketId === s.id);
        if (!r || !p)
            return;
        p.connected = false;
        ...
        broadcast(r);
        setTimeout(() => {
            if (r.players.every((q) => !q.connected))
                rooms.delete(r.code);
        }, 120000);
    });
});
